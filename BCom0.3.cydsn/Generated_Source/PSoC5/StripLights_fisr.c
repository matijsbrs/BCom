...
void StripLights_fisr_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)StripLights_fisr__INTC_NUMBER] = address;
}
...
cyisraddress StripLights_fisr_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)StripLights_fisr__INTC_NUMBER];
}
...
{
    *StripLights_fisr_INTC_PRIOR = priority << 5;
}
...
uint8 StripLights_fisr_GetPriority(void)
{
    ...
    priority = *StripLights_fisr_INTC_PRIOR >> 5;
